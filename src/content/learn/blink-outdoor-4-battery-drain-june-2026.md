...
The [Outdoor 4](https://quvii.com/learn/blink-outdoor-4-motion-detection-delay) introduced on-device Person Detection, which is available with a <a href="https://www.amazon.com/s?k=Blink+Subscription+Plan&tag=jeffzen911-20" rel="sponsored nofollow noopener" target="_blank">Blink Subscription Plan</a>. While this feature reduces "nuisance" alerts (like swaying trees), the initial motion trigger still wakes the camera's processor. The camera must then run a computer vision algorithm to determine if the motion was a person. This extra processing "tax" occurs for every motion event, even if you never receive a notification. In high-traffic areas, the AI overhead can reduce battery life by 15-20% compared to standard PIR [motion detection](https://quvii.com/learn/reolink-camera-motion-detection-false-alerts-fix).

### Signal Interference in Modern Smart Homes
The #1 silent killer of Blink batteries is "Network Friction." The [Outdoor 4](https://quvii.com/learn/blink-outdoor-4-motion-detection-delay) uses two radios: a Low Frequency Radio (LFR) to talk to the Sync Module and a 2.4GHz Wi-Fi radio to upload video. If the Wi-Fi signal is weak (indicated by 1 or 2 bars in the app), the camera must increase its transmission power and stay "awake" longer to successfully upload a clip. In 2026, with the proliferation of Wi-Fi 6E and Wi-Fi 7 devices, the 2.4GHz band is increasingly congested, forcing the camera to perform more re-tries, which exponentially drains the lithium cells.
...
- RTINGS — [Blink Outdoor 4](https://quvii.com/learn/blink-outdoor-4-motion-detection-delay) Review and Power Testing