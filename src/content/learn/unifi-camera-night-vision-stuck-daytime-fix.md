...
A [UniFi camera](https://quvii.com/blog/ring-vs-ubiquiti-unifi-comparison-2026)’s night vision being "stuck daytime" refers to a failure in the automated transition between the camera’s color-accurate daytime mode and its infrared-sensitive night mode. In a functioning system, the camera uses an Ambient Light Sensor (ALS) to detect when light levels drop below a certain lux threshold. At that point, the camera should physically move an IR-cut filter out of the way and activate its onboard Infrared (IR) LEDs.
