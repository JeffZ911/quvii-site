...
The landscape of [motion detection](https://quvii.com/blog/arlo-doorbell-missing-motion-detection-alerts) has diverged into two distinct categories: simple movement detection and true "presence" sensing. A traditional "dumb" sensor, typically using Passive Infrared (PIR) technology, only knows if something warmer than the background moved across its field of view. In contrast, a 2026 "smart" motion detector uses high-frequency radar or local computer vision to understand *what* is moving and *where* it is located within a room.
...
[Motion detection](https://quvii.com/blog/arlo-doorbell-missing-motion-detection-alerts) is transient; it captures the act of moving. Presence sensing is state-based; it captures the fact of being there. For security, this distinction is vital. A [motion sensor](https://quvii.com/blog/ring-mqtt-reolink-motion-sensor-integration-fix) might trigger an entry alert, but a presence sensor can confirm if an intruder is still hiding in a room, even if they remain perfectly still.
