...
Crucially, the integration parses the *text* and *metadata* of the notification to determine the state of the sensor. For example, if the notification says "Person detected at Front Door," the integration flips the `binary_sensor` to "on." If [push notifications](https://quvii.com/blog/eufy-motion-push-notifications-failing-july-2026) are disabled in the Eufy app, or if the notification style is set to "Basic" (no thumbnail/detail), the FCM packet may lack the specific metadata the integration needs to identify which camera triggered.
