...
Owners of various models—ranging from the Ring Video Doorbell [Pro 2](https://quvii.com/blog/arlo-pro-2-cloud-storage-recording-issues) to the Stick Up Cam Pro—are describing a systemic failure in the notification pipeline. Specifically, cameras are failing to push alerts to smartphones, and in many cases, the "Event History" shows gaps where motion clearly occurred. When users attempt to view these events, they are often met with a "Footage Unavailable" or "Streaming Error" message.
...
3.  **Check Event History:** If the event shows up in the timeline but never sent a notification, check your phone’s "Do Not Disturb" settings and the app’s "[Motion Alerts](https://quvii.com/blog/eufy-p2p-motion-alerts-broken-fix)" toggle.
